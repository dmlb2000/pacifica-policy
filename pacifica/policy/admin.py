...
class AdminPolicy(object):
    ...
    def get_relationship_info(self, **get_args):
        ...
        return loads(requests_retry_session().get(self._format_url('all_relationships_url', **get_args)).text)

    def _all_project_info(self):
        return loads(requests_retry_session().get(self._format_url('all_projects_url')).text)

    def _all_instrument_info(self):
        return loads(requests_retry_session().get(self._format_url('all_instruments_url')).text)
    ...
    def _projects_for_user(self, user_id, relationship='member_of'):
        ...
        return [part['project'] for part in loads(requests_retry_session().get(proj_url).text)]
    ...
    def _instruments_for_custodian(self, user_id):
        ...
        inst_custodian_list = loads(requests_retry_session().get(
    ...
    def _projects_for_inst(self, inst_id):
        ...
        inst_projs = loads(requests_retry_session().get(inst_projs_url).text)
    ...
    def _project_info_from_ids(self, proj_list):
        ...
        if proj_list:
            for proj_id in proj_list:
                ...
                ret.extend(loads(requests_retry_session().get(proj_url).text))
        return ret

    def _groups_for_inst(self, inst_id):
        inst_g_url = self._format_url('inst_group_url', instrument_id=inst_id)
        return [i['group'] for i in loads(requests_retry_session().get(inst_g_url).text)]

    def _instruments_for_group(self, group_id):
        inst_g_url = self._format_url('inst_group_url', group_id=group_id)
        return [i['instrument'] for i in loads(requests_retry_session().get(inst_g_url).text)]
    ...
    def _instruments_for_user_proj(self, user_id, proj_id):
        ...
        proj_insts = set([part['instrument']
                          for part in loads(requests_retry_session().get(proj_insts_url).text)])
        inst_groups = set()
    ...
    def _instrument_info_from_ids(self, inst_list):
        ...
        for inst_id in inst_list:
            ...
            ret.extend(loads(requests_retry_session().get(inst_url).text))
        return ret

    def _users_for_proj(self, proj_id):
        user_proj_url = self._format_url(
            'proj_user_url', project=proj_id)
        user_projs = loads(requests_retry_session().get(user_proj_url).text)
        return list(set([str(part['user']) for part in user_projs]))

    def _user_info_from_kwds(self, **kwds):
        return loads(requests_retry_session().get(self._format_url('all_users_url', **kwds)).text)
    ...
    def _object_id_valid(object_lookup_name, object_id):
        ...
        object_value_req = requests_retry_session().get(object_query_url)
    ...
    def _is_admin(self, user_id):
        ...
        is_admin_req = requests_retry_session().get(amember_query)
